...
mlp = MLPClassifier(hidden_layer_sizes=(1000,), max_iter=10000,solver='sgd', verbose=True, tol=1e-4, learning_rate_init=0.0001)
...
for train_index, test_index in kf.split(lista_dados_test):
	...
	mlp.fit(X_train, y_train)
	print("Current KFolds score: %f" % mlp.score(X_train, y_train))
...
for i in range(len(pesos)):
	savetxt('pesos.csv', pesos[i], delimiter=',')
